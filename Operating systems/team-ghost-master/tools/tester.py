...
def run_kernel_test(test_descriptor, extra_configure_args):
    ...
    memory_size = None
    extra_kernel_defines = []

    if len(parts) > 1:
        for part in parts[1:]:
            if part.startswith('m'):
                memory_size = int(part[1:])
            elif part.startswith('kp'):
                sizes = part[2:].split("-")
                extra_kernel_defines.append('KERNEL_PADDING_HEAD=' + sizes[0])
                extra_kernel_defines.append('KERNEL_PADDING_TAIL=' + sizes[1])
            elif part.startswith('kq'):
                extra_kernel_defines.append('KERNEL_SCHEDULER_QUANTUM=' + part[2:])
            elif part.startswith('D'):
                extra_kernel_defines.append(part[1:])
    ...
    for i in extra_kernel_defines:
        configure_args.append('--kernel-cflag={}'.format(i))
    ...
    if should_panic:
        if not actually_panicked:
            raise TesterException('test failed (did not panick)', 'see msim.log')
    elif not found_test_passed:
...
def main():
    ...
    report = run_tests(tests, config.fail_fast, config.configure_arg)
