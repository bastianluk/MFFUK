...
-- rle :: Eq a => [a] -> [(Int, a)]
-- rle xs = map (\sled -> (length sled, head sled)) (group (filter odd (map (^2) xs)))
-- rle = udelejto
--     where nadruhou = map (^2)
--           add_len = map (\sled -> (length sled, head sled))
--           udelejto = (add_len . group . (filter odd) . nadruhou)
...
absurdniVypocet = ceiling . negate . tan . cos
-- TODO: přepište rle pomocí (.) - skládání funkcí

add_len :: [a] -> (Int,a)
add_len array = (length array, head array)
 
map_add_len :: [[a]] -> [(Int, a)]
map_add_len list = map add_len list
 
rle :: Eq a => [a] -> [(Int,a)]
rle = map_add_len . group



append_flat :: [a] -> [a] -> [a]
append_flat to from = to ++ from
...
data NumX a = NegInfGeneric | PosInfGeneric | NumGeneric a deriving (Show)
...
ints = map prevedNaInt $ filter isNumber intxs
    where
        -- isNumber x = case x of 
        --     (Num _) -> True
        --     _ -> False
        isNumber (Num _) = True
        isNumber NegInf = False
        isNumber PosInf = False
        prevedNaInt (Num x) = x
        prevedNaInt _ = error "to je blbost"
